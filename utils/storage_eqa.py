...
class RolloutStorage(object):
    ...
    def __init__(self, num_steps, num_processes, obs_shape, action_space,
        ...
        self.actions = torch.zeros((num_steps, num_processes, self.n_actions),
                                   dtype=action_type)
        self.pred_dists = torch.zeros(num_steps, num_processes) # ADDED
        self.distance_to_goal = torch.zeros(num_steps, num_processes) # ADDED
    ...
    def to(self, device):
        ...
        self.pred_dists = self.pred_dists.to(device) # ADDED
        self.distance_to_goal = self.distance_to_goal.to(device) # ADDED
    ...
    def insert(self, obs, rec_states, actions, action_log_probs, value_preds,
               rewards, pred_dists, distance_to_goal,masks):
    # def insert(self, obs, rec_states, actions, action_log_probs, value_preds,
    #            rewards, masks):
        ...
        self.pred_dists[self.step].copy_(pred_dists) # ADDED
        self.distance_to_goal[self.step].copy_(distance_to_goal) # ADDED
    ...
    def feed_forward_generator(self, advantages, num_mini_batch):
        ...
        for indices in sampler:
            yield {
                ...
                "pred_dists": self.pred_dists.view(-1)[indices], # ADDED
                "distance_to_goal": self.distance_to_goal.view(-1)[indices], # ADDED
    ...
    def recurrent_generator(self, advantages, num_mini_batch):
        ...
        for start_ind in range(0, num_processes, num_envs_per_batch):
            ...
            for offset in range(num_envs_per_batch):
                ...
                pred_dists.append(self.pred_dists[:, ind])# ADDED
                distance_to_goal.append(self.distance_to_goal[:,ind]) # ADDED
            ...
            pred_dists = torch.stack(pred_dists, 1)# ADDED
            distance_to_goal = torch.stack(distance_to_goal, 1) # ADDED
            ...
            yield {
                ...
                'pred_dists': _flatten_helper(T, N, pred_dists),# ADDED
                "distance_to_goal": _flatten_helper(T, N, distance_to_goal), # ADDED 
                'rec_states': torch.stack(rec_states, 1).view(N, -1),
            }


class EQAGlobalRolloutStorage(RolloutStorage):

    def __init__(self, num_steps, num_processes, obs_shape, action_space,
                 rec_state_size, extras_size):
        super(EQAGlobalRolloutStorage, self).__init__(
    ...
    def insert(self, obs, rec_states, actions, action_log_probs, value_preds,
               rewards, pred_dists, distance_to_goal,masks,extras):
        self.extras[self.step + 1].copy_(extras)
        super(EQAGlobalRolloutStorage, self).insert(
            obs, rec_states, actions,
            action_log_probs, value_preds, rewards, pred_dists, distance_to_goal, masks)
