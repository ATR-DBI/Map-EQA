...
from agents.eqa_zeroshot import EQA_Zeroshot_Env_Agent
from agents.vqa import VQA_Env_Agent
...
def make_env_eqa_fn(args, config_env, rank):
    print("Make dataset:")
    print(config_env.DATASET.TYPE, config_env.DATASET)
    # print(f"############{config_env.DATASET.SCENES}")
    dataset = make_dataset(config_env.DATASET.TYPE, config=config_env.DATASET)
    config_env.defrost()
    config_env.SIMULATOR.SCENE = dataset.episodes[0].scene_id
    config_env.freeze()
    # print(f"######{config_env.SIMULATOR.SCENE}")
    env = EQA_Zeroshot_Env_Agent(args=args, rank=rank,
                            config_env=config_env,
                            dataset=dataset)
    env.seed(rank)
    return env

def make_env_eqa_vqa_fn(args, config_env, rank):
    print("Make dataset:")
    print(config_env.DATASET.TYPE, config_env.DATASET)
    dataset = make_dataset(config_env.DATASET.TYPE, config=config_env.DATASET)
    config_env.defrost()
    config_env.SIMULATOR.SCENE = dataset.episodes[0].scene_id
    config_env.freeze()
    env = VQA_Env_Agent(args=args, rank=rank,
                            config_env=config_env,
                            dataset=dataset
                            )
...
def _get_scenes_from_folder_mp3d(content_dir):
...
def construct_envs(args):
    ...
    if "*" in basic_config.DATASET.CONTENT_SCENES:
        ...
        elif args.task_config=='tasks/objectnav_mp3d.yaml':
            # print("MP3D")
            scenes = _get_scenes_from_folder_mp3d(content_dir)
        print(scenes)
    ...
    for i in range(args.num_processes):
        ...
        agent_sensors = config_env.SIMULATOR.AGENT_0.SENSORS # Equirectangularを使うかどうかを.yamlで決める
        # print(agent_sensors)
        ...
        # config_env.ENVIRONMENT.MAX_EPISODE_STEPS = 1000
...
def construct_eqa_envs(args, make_env_fn=make_env_eqa_fn):
    env_configs = []
    args_list = []
    ...
    basic_config.DATASET.EPISODES_DIR = args.episode_dir
    # basic_config.DATASET.SCENE_DIR = args.scene_dir
    basic_config.freeze()


    # if args.eval==0:
    content_dir = args.content_dir.format(split=args.split)
