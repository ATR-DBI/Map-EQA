...
GET_FRONTIERS = "get_frontier_map"
...
class VectorEnv:
            ...
            while command != CLOSE_COMMAND:
                if command == STEP_COMMAND:
                    ...
                    if isinstance(env, habitat.RLEnv) or isinstance(
                        ...
                        # habitat.RLEnv
                        observations, reward, done, _info = env.step(**data)
                        info=_info.copy()
                        if auto_reset_done and done:
                            observations, _info = env.reset() ##TODO changed info to _info
                ...
                # TODO: update CALL_COMMAND for getting attribute like this
                    ...
                    observations, reward, done, _info = \
                    ...
                    result = env.get_obs()
                    connection_write_fn(result)
                elif command == GET_FRONTIERS:
                    result = env.get_frontier_map(data)
                    connection_write_fn(result)
                else:
                    raise NotImplementedError
    ...
    def get_frontier_map(self, planner_inputs):
        self._is_waiting = True
        # for write_fn in self._connection_write_fns:
        #     write_fn((GET_FRONTIERS, None))
        for write_fn, args in zip(self._connection_write_fns, planner_inputs):
            write_fn((GET_FRONTIERS, args))
        results = []
        for i, read_fn in enumerate(self._connection_read_fns):
            # print(planner_inputs)
            # print(read_fn)
            # import pdb;pdb.pdb.set_trace()
            # results.append(read_fn(planner_inputs[i]))
            results.append(read_fn())
    ...
    def plan_act_and_preprocess(self, inputs):
        ...
        self._is_waiting = False
        # import pdb;pdb.pdb.set_trace()
        # self.logger.info(f"{[ob.shape for ob in obs]}, {[done for done in dones]}, {[rew for rew in rews]}")
        # print(f"{[ob.shape for ob in obs]}, {[done for done in dones]}, {[rew for rew in rews]}")
        return np.stack(obs), np.stack(rews), np.stack(dones), infos
