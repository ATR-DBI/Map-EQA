...
from constants import coco_categories_mapping, coco2mp3d_categories_mapping, restrictedmp3d2idx
...
class SemanticPredMaskRCNN():
    ...
    def get_prediction(self, img):
        ...
        # semantic_input = np.zeros((img.shape[0], img.shape[1], 7 + 1))

        for j, class_idx in enumerate(
                seg_predictions[0]['instances'].pred_classes.cpu().numpy()):
            # if class_idx in list(coco2mp3d_categories_mapping.keys()):
            #     mpidx = coco2mp3d_categories_mapping[class_idx]
            #     idx = restrictedmp3d2idx[mpidx]
...
class SemanticPredMaskRCNN_mp3d():
